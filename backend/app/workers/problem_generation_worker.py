...
# Enough context to steer away from repeats without making every prompt huge
MAX_AVOID_PROBLEMS = 30
...
async def generate_problem_for_section(
    ...
    # Listing the section's existing problems keeps new ones from repeating them
    avoid_text = ""
    if avoid:
        listed = "\n".join(f"- {problem}" for problem in avoid[-MAX_AVOID_PROBLEMS:])
        avoid_text = f"Write a problem that is clearly different from these existing ones:\n{listed}\n"
...
{avoid_text}Write every mathematical expression in LaTeX wrapped in $...$ (for example $x^2 + \\\\frac{{1}}{{2}}$). Because your reply is JSON, escape every backslash as a double backslash.
